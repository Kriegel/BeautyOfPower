...
Function Get-PSOneToken {
  ...
  (
    ...
    # see all avaiable TokenKinds with this command: [Enum]::GetNames([System.Management.Automation.Language.TokenKind])
    [System.Management.Automation.Language.TokenKind[]]
    $TokenKind = $null,

    # the kind of token requested. If neither TokenKind nor TokenFlag is requested,
    # a full tokenization occurs
    # see all avaiable TokenKinds with this command: [Enum]::GetNames([System.Management.Automation.Language.TokenFlags])
  ...
  process
  {
    ...
    if ($returnTokens)
    {
      ...
      $tokens |
      & { process { if ($null -eq $TokenKind -or
          $TokenKind -contains $_.Kind)
          { $_ }
      }} |
      & { process {
          $token = $_
          if ($null -eq $TokenFlag) { $token }
          else {
            $TokenFlag |
            Foreach-Object {
